            ...
            for book in all_books:
                if (jsonData == str(book[_field])):
                    ...
                    if len(dict_jsonData) == 0:
                        dict_jsonData = {jsonData: [
                            jsonData_dict['id'], counter]}
...
def AboutView(request):
    auth_person = None  # auth_Person_Function(str(request.user))
    isServerLocal = isServerLocalFunction()
...
def TopicsView(request):
    ...
    if request.method == 'POST':
        _searchWord = request.POST['searchWord']
        temp_list = final_list[2]
        final_list[2] = []

        for temp_dict in temp_list:
            for key, value in temp_dict.items():
                if _searchWord.lower() in key.lower():
                    final_list[2].append({key: value})
